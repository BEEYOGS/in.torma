'use server';
/**
 * @fileOverview Converts text to speech using Genkit and the Gemini 2.5 Flash TTS model.
 *
...
const textToSpeechFlow = ai.defineFlow(
  ...
  async (query) => {
    // If the input query is empty or just whitespace, return an empty audio URI to prevent errors.
    if (!query || !query.trim()) {
      return {
        media: 'data:audio/wav;base64,',
      };
    }
